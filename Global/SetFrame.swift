...
extension UIView {
    ...
    func setFrame(centerX: CGFloat, bottom: CGFloat, width: CGFloat, height:CGFloat) { // 勿改
        self.frame.size = CGSize(width: width, height: height)
        self.centerX = centerX
        self.frame.origin.y = self.superview!.height - self.height - bottom
    }
    ...
    func setFrame(right: CGFloat, bottom: CGFloat, width: CGFloat, height:CGFloat) {
        ...
        self.frame.origin.x = self.superview!.width - self.width - right
        self.frame.origin.y = self.superview!.height - self.height - bottom
        // 如果直接添加到View，(有时候)要多加 98pt，如果有标签栏就再加83pt(为181pt)
        // 如离标签栏20pt的悬浮按钮：setFrame(right: 20, bottom: 181 + 20, width: 60, height: 60)
    }
