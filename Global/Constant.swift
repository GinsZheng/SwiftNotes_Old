...
// MARK: - 设备信息
let kDeviceName = UIDevice.current.name // 设备名称，如Gins

let kDeviceModel = UIDevice.modelName  // 获取设备型号(iPhone 11，iPhone 13 Pro Max)

let kSystemVersion = UIDevice.current.systemVersion // 如 15.2

let kIdentifierForVendor = UIDevice.current.identifierForVendor  // 虚拟id(IDFV)

let kModelType = UIDevice.current.model // e.g. iPhone, iPad

let kAppVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] ?? ""

let kBundleName = Bundle.main.infoDictionary?["CFBundleVersion"] ?? ""
